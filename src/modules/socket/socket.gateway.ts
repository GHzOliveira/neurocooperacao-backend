...
async handleCreateGame(client: Socket, groupId: string) {
  const group = await this.prisma.grupo.findUnique({ where: { id: groupId } });
...
handleMessage(client: Socket, payload: string, groupId: string): void {
  ...
  if (message) {
    console.log(`Emitindo 'storedMessage' para o cliente '${client.id}' com a mensagem: '${message}', `);
...
async handleGameRule(client: Socket, [gameRule, groupId]: [string, string]): Promise<void> {
  ...
  try {
    ...
    console.log(`Regra do jogo armazenada no banco de dados para a rodada ${groupId}`);
    this.server.emit('gameRuleStored', gameRule);
  } catch (error) {
    console.error(`Erro ao armazenar a regra do jogo no banco de dados: ${error}`);
...
@SubscribeMessage('joinGame')
  handleJoinGame(client: Socket, groupId: string): void {
    ...
    console.log(`Usuário ${client.id} entrou no servidor de jogo do grupo ${groupId}`);
  }
